...
class TestCachedDataMetadataMethods(TestCase):
    def setUp(self):
        ...
        self.expected_contexts = ["yxdb", "sqlite", "pandas", "python"]
        ...
        for from_context_name in contexts:
            ...
            for from_field_type_name in from_context:
                ...
                if "conversion_types" not in from_field_type:
                    raise LookupError(
                        "missing conversion_types attribute in field {}".format(
                            from_field_type_name
                        )
                    )
                for to_context_name in from_field_type["conversion_types"]:
                    to_field_type_name_list = from_field_type["conversion_types"][
                        to_context_name
                    ]
                    for field_type_name in to_field_type_name_list:
                        ...
                        try:
                            if to_context_name not in contexts:
                                raise LookupError(
                                    'context "{}" does not exist'.format(
                                        to_context_name
                                    )
                                )
                                everything_matches = False
                            if field_type_name not in contexts[to_context_name]:
                                print(contexts[to_context_name])
                                raise LookupError(
                                    "field {} does not have a mapping for {}->{}".format(
                                        field_type_name,
                                        from_context_name,
                                        to_context_name,
                                    )
                            ...
                            print(
                                "from context: {}, from field type: {}, to context: {}, to field type: {}".format(
                                    from_context_name,
                                    from_field_type_name,
                                    to_context_name,
                                    field_type_name,
    ...
    def testValuesInContext(self):
        field_check_outcome = self.checkFieldLookups(
            self.metadata_tools.columns["context"]
        )
    ...
    def testPartialFieldConversionMap(self):
        ...
        self.metadata_tools.columns["context"]["yxdb"][fake_ayx_field_type] = {
            "conversion_types": {"sqlite": [fake_sqlite_field_type]},
        ...
        if field_check_outcome["isvalid"]:
            self.fail(
                "this should cause an error (due to no reverse lookup existing for field {})".format(
                    fake_ayx_field_type
                )
            )
        else:
    ...
    def testCompleteFieldConversionMap(self):
        ...
        self.metadata_tools.columns["context"]["sqlite"][fake_sqlite_field_type] = {
            "conversion_types": {"yxdb": [fake_ayx_field_type]},
            "expected_length_dim": 1,
            "default_length": (2147483647,),
        }
        # does everything look good? it shouldn't...
